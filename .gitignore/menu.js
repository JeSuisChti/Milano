...
bot.on("message", async(message) => {
  ...
  var help_invite = new Discord.RichEmbed()
    ...
    var help = new Discord.RichEmbed()
      ...
      .addField("➜ **Commandes funs**", "`bowling` ; `code-binaire` ; `roulette` ; `uncode-binaire` ; `server` ; `emoji` ; `cat` ; `dog` ; `pileouface` ; `succes` ; `profile` ; `avatar` ; `info`")
      ...
      .then(async msg => {
        ...
        bot.on("messageReactionAdd", (reaction, user) => {
          ...
          if(reaction.emoji.id === "595977330632032276" && bot.user.id != user.id && reaction.message.id === msg.id) {
            ...
            if(inscrit_list.length <= 1) return message.channel.send("<:error:596647262680121373>Tu ne peux pas lancer la partie tout seul ! 😶").then(reaction.remove(user)).then(msg => msg.delete(5000))
    ...
    if(message.content.startsWith(prefix + "help")) {
      ...
      if(message.content.startsWith(prefix + "info")) {
        ...
        var serv_embed = new Discord.RichEmbed()
          ...
          .addField("__**Emojis**__", Message , true)
          .addField("__**Rôles (" + message.guild.roles.map(r => r.name).length + ")**__", MessageRole , true)
          .setTimestamp()
